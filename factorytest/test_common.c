...
int app_dd_speed_test(char *path, char *outputfile)
{
	...
	if(outputfile == NULL)
	{
		...
		system("sudo time dd if=/dev/zero of=./largefile bs=64k count=16k conv=fsync");// oflag=dsync
		printf(" \r\n");
	...
	else
	{
		...
		sprintf(buf, "sudo dd if=/dev/zero of=./largefile bs=64k count=16k conv=fsync 2>>%s", outputfile);	
		system(buf);
